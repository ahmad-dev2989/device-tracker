...
const CapacitorSecureStorage = Capacitor.isNativePlatform()
  ? (await import("@capacitor/core")).registerPlugin<any>("SecureStorage")
  : null;
...
export const secureStore = {
  ...
  async set(key: string, value: string): Promise<void> {
    if (isTauri()) {
      ...
      console.warn(
        `[SecureStorage] Unsafe Fallback: Storing credential key "${key}" in LocalStorage for simulation.`
      );
  ...
  async get(key: string): Promise<string | null> {
    if (isTauri()) {
      try {
        const { invoke } = await import("@tauri-apps/api/core");
        const val = await invoke<string | null>("get_secure_credential", { key });
        return val;
      } catch (e) {
